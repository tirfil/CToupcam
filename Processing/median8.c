...
struct uchar_node {
	unsigned char 		value;
	struct uchar_node 	*next;
};

typedef struct uchar_node uchar_node;

long width=0;
long height=0;
uchar_node** root = NULL;
...
void write_output_fits(char* filename, unsigned char* imageout){
	fitsfile *fptrout;
	...
	fits_write_img(fptrout, TBYTE, 1, nelements, imageout, &status);
...
int main(int argc, char* argv[]) {
	...
	imageout = malloc(sizeof(unsigned char)*nelements);
	
	while(start < nelements){
		printf("Processing start=%d stop=%d\n",start,stop);
		cluster = stop-start;
		root = malloc(sizeof(uchar_node*)*cluster);
